...
**Linux namespace sandbox**: Fork child → child calls `unshare(CLONE_NEWUSER)` → parent writes identity UID/GID map (`uid uid 1` / `gid gid 1`) to `/proc/<child>/{setgroups,uid_map,gid_map}` → child calls `unshare(CLONE_NEWNS)`, sets mount propagation private (`MS_REC|MS_PRIVATE`), bind-mounts empty dirs over credential paths (per mode), scrubs sensitive env vars (`AWS_SECRET*`, `SSH_AUTH_SOCK`, etc.), and execs the agent. Two-pipe synchronization ensures correct ordering. The child retains the real UID/GID so all toolchains (JVM ByteBuddy, Gradle, npm, etc.) work without workarounds. Implemented as a Python launcher script (`_build_launcher_script()`) spawned by `namespace_argv()`. The masked set includes the governance keystone (`security_policy.json`, `admission_policy.json`, `computer_use.json`, `denied_commands.json`, `profiles`, `token_signing.key` — `sandbox._KEYSTONE_LEAVES`, which records what is deliberately left out, chiefly the SEL trust root; see [sel](sel.md)) under both crew home prefixes, in every tier — and the macOS Seatbelt profile (`_build_seatbelt_profile()`) denies the same paths. The `$HOME`-relative entries are DERIVED once at module load (`sandbox._KEYSTONE_HOME_ENTRIES`, the cross product of `_CREW_HOME_PREFIXES` × `_KEYSTONE_LEAVES`) and spliced into all three tier lists, the shape `security._SENSITIVE_HOME_DIRS` already uses for `_CREW_SECRET_LEAVES` — three hand-written copies of the same entries are three chances for one tier to lose a leaf, silently and in the permissive direction. That is a module-load comprehension, NOT a runtime wiring of `sensitive_home_dirs()`, which would put a per-entry host stat on the event loop for every async spawn. This is a distinct mechanism from `is_sensitive_path()`: that gate covers the agent's in-process tool calls, while a spawned subprocess reaches these paths through an OS `open()` that never routes through it, so the launcher's bind-mount list (and the Seatbelt deny) must hold the keystone independently. A bind-mount of an empty inode over the target hides the real file from read and write alike, so a sandboxed spawn can neither read an existing ceiling file nor rewrite it, and it cannot unlink or rename over the mount point either (both return `EBUSY`).

**Residual, Linux only — an ABSENT keystone leaf can still be CREATED.** A bind-mount needs an existing target, so the launcher's `isdir`/`isfile` guards skip a path that is not there. This is a ceiling *raise*, not mere disclosure, and it predates the mask (which only narrows it: an existing leaf is now closed in both directions). Two leaves carry real harm, and the bound is measured: an agent-created `denied_commands.json` with `disable_all` takes the effective built-in deny set to **empty** on any host with no governance ceiling — `compute_effective_denied`'s `governance_pins` is empty on a standalone install, so nothing pins a rule back — and `computer_use.json` is re-read on every dispatch, so `enabled: true` grants desktop control where the operator never opted in. Closing it means materializing an absent leaf before the mask is built, which is deliberately deferred for three per-leaf reasons: (1) `security_policy.json` has **no inert parseable spelling** — `{}` and `{"version": 1}` both raise `PlatformCompositionError` out of `parse_policy` (boot aborts), and anything that does parse flips `ctx.governance` from `None` to a real ceiling product-wide; (2) `token_signing.key` is a 32-byte HMAC secret under a documented no-eager-create discipline (`dashboard/token_secret.py`) whose eager write breaks `gateway --seed` and pollutes the home for read-only CLI commands, and short/junk bytes fall back to an ephemeral secret that invalidates every outstanding cookie and channel link; (3) there is no shared choke point yet — `wrap_argv` is called from ~29 modules, several outside the gateway process (the cron script child, CLI setup), so a gateway-start step would leave those spawns uncovered while reading as complete. A pre-created **empty** file is separately wrong: it is not valid JSON, and although every reader fails soft to DISABLED, the dashboard's mutation path (`_read_denied_strict`) raises rather than clobber an unparseable ceiling, so the mask would lock the operator out of their own keystone. The follow-up is to materialize the two leaves that DO have an inert parseable default (`denied_commands.json`, `computer_use.json`) at a choke point every spawn shares. macOS has no such residual — Seatbelt's denies are path-based and cover a path that does not exist yet.

The tier-list entries are `$HOME`-relative, so a relocated `KIROCREW_HOME` would move the data home out from under them; `_relocated_keystone_dirs()` (mirroring `_relocated_policy_cache_dirs()`) additionally masks each keystone leaf under the resolved data home whenever it differs, so the launcher and Seatbelt keep the keystone masked on a relocated home — matching the in-process gate, which re-anchors the same leaves under the resolved `KIROCREW_HOME` in `security._home_dir_targets_uncached()`. It resolves with `normpath`, not `realpath`, because it runs on the event loop for every async spawn and a link-resolving syscall on a stalled home would freeze the gateway. On macOS the Seatbelt profile carries an explicit `file-write*` deny for the keystone leaves as well as the read and hardlink denies (Linux closes both directions through the single empty-inode bind-mount, but a path-based read deny alone leaves the write direction open) — scoped to the keystone and the cache/voice-runtime execution roots, never widened to credential dirs like `~/.aws` that legitimately refresh state.
