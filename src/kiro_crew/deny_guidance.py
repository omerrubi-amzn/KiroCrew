...
REMEDIATION: dict[str, str] = {
    ...
    DENY_CLASS_SECRET_FILE: (
        ...
        "aws, git and ssh each resolve their own credentials. When the refused thing "
        "was a command that would have obtained a credential, the supported route is "
        "a configured profile whose `credential_process` vends one on demand, or the "
        "credential-vending tool this host provides — both of which are the user's "
