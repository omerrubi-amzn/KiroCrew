...
#: "the directory is non-empty", which cannot tell a complete tree from one an
#: interrupted install left half-written.
...
def node_modules_matches_lockfile(git: str, repo: str, ref: str) -> bool:
    ...
    return _sha256(worktree_lock) == incoming_hash and _sha256(hidden_lock) == incoming_hash
