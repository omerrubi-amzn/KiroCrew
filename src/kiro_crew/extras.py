...
def install_hint(extra: str) -> str:
    ...
    if not reqs:
        return ""
    return "pip install " + " ".join(shlex.quote(r) for r in reqs)
...
def pip_install_command(extra: str) -> str:
    ...
    if os.name == "nt":
        ...
        # Requirement specifiers hold `<`, `>` and `,` -- redirection operators
        # in both shells -- so each one is single-quoted here too.
        specs = " ".join("'" + r.replace("'", "''") + "'" for r in reqs)
        return f"& '{exe}' -m pip install {specs}"
    specs = " ".join(shlex.quote(r) for r in reqs)
