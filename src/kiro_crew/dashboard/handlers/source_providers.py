...
# exists to avoid. Backslash-escaping these keeps ADF text literal, leaving the
...
def _adf_inline_sequence(children: list[dict[str, Any]], *, _depth: int) -> str:
    """Render a run of inline nodes, keeping a split credential contiguous.

    A plain-text walk joins sibling text nodes seamlessly, so the payload-level
    ``_redact_provider_data`` pass sees one string and catches a credential that
    spans two of them -- ``ghp_...`` in one node and its tail in a bold sibling.
    Marks and escaping put delimiters between those halves, which would hide the
    secret from that pass. So when a run's own raw text carries a credential, the
    run is emitted as one redacted string and gives up its formatting: the run is
    the exact span where a seamless walk and this converter disagree, which is
    why the check lives here rather than over the whole document (a secret split
    across two BLOCKS was never contiguous, in this converter or before it).
    """
    raw = "".join(_adf_raw_text(child, _depth=_depth) for child in children)
    redacted = str(_redact_provider_data(raw))
    if redacted != raw:
        return _md_escape_inline(redacted)
    return "".join(_adf_inline_to_markdown(child, _depth=_depth) for child in children)
