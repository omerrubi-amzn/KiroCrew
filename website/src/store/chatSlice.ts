...
export const refreshSlot = createAsyncThunk(
  ...
  async (key: string, { getState }) => {
    ...
    const viewCounts = midOccurrences(view)
    const pageCounts = midOccurrences(page.messages)
    const anchors = (id: unknown): boolean =>
      idAnchorsOneRow(id, view, page.messages, viewCounts, pageCounts, { requireTs: true })
    const spansView = anchors(serverRows[0].meta?.mid)
