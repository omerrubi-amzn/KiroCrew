...
export function useVirtualChat<T>(
  ...
  if (rowDeparturePossible) {
    ...
    //
    // Head paging is excluded HERE, on the shape of the departure itself rather
    // than on a proxy: trimming the head drops a contiguous PREFIX and leaves
    // the rest standing, and those rows are coming back, so their measurements
    // must keep pricing the region the reader is about to page into. Every other
    // departure retires. A full clear departs everything and leaves NO survivor,
    // which is why the survivor test is what separates a wipe from a page-out --
    // both rename index 0, and only one of them is coming back.
    //
    // A single row leaving the very head is indistinguishable from a one-row
    // page-out, so it is skipped, as it was before this branch existed.
    ...
    const headPagedOut = departedPrefixOnly && survivorSeen
