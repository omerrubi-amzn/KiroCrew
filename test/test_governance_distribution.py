...
class TestTheCacheIsHiddenFromAgentSubprocesses:
    ...
    def test_the_crew_home_prefixes_match_the_security_floor(self):
        """``_KEYSTONE_HOME_ENTRIES`` is derived over these, so a prefix the security floor
        fences but the launcher does not spell would be a silently narrower OS-level mask —
        and the legacy ``~/.kirocrew`` twin is exactly the one that would go missing."""
        from kiro_crew import sandbox

        assert tuple(sandbox._CREW_HOME_PREFIXES) == tuple(security._CREW_HOME_PREFIXES)

    def test_the_keystone_entries_are_derived_not_respelled(self):
        """The tier lists splice ``_KEYSTONE_HOME_ENTRIES`` rather than carrying three
        hand-written copies of the same 12 strings, so pin that it stays the full cross
        product of prefixes x leaves. Re-spelling the entries per tier would restore the
        drift class this change closes: three copies are three chances for one tier to
        lose a leaf, and the loss is silent and in the permissive direction.
        """
        from kiro_crew import sandbox

        assert sandbox._KEYSTONE_HOME_ENTRIES == [
            f"{prefix}/{leaf}"
            for prefix in sandbox._CREW_HOME_PREFIXES
            for leaf in sandbox._KEYSTONE_LEAVES
        ]
        # No duplicates, and every tier carries the derived block exactly once.
        assert len(set(sandbox._KEYSTONE_HOME_ENTRIES)) == len(sandbox._KEYSTONE_HOME_ENTRIES)
        for dirs in (sandbox._STRICT_DIRS, sandbox._STANDARD_DIRS, sandbox._CC_DIRS):
            for entry in sandbox._KEYSTONE_HOME_ENTRIES:
                assert dirs.count(entry) == 1

    def test_the_ceiling_document_has_no_inert_parseable_spelling(self):
        """Why the absent-target residual is NOT closed by pre-creating the leaf, pinned.

        The obvious remedy for "a bind-mount needs an existing target" is to materialize an
        absent leaf with a parseable safe default. That works for the two opt-out leaves,
        whose readers treat ``{}`` as "no opt-out" / DISABLED — but NOT for the ceiling
        document, and this is the claim ``_KEYSTONE_HOME_ENTRIES`` records. Every parseable
        spelling of ``security_policy.json`` IS a governed ceiling: the two candidate inert
        forms fail closed out of ``parse_policy``, and anything that parses flips
        ``ctx.governance`` from ``None`` product-wide. Pinned because if ``parse_policy``
        ever accepts ``{}``, the documented reason evaporates silently and the next reader
        would be entitled to materialize the file.
        """
        from kiro_crew.platform import governance as gov
        from kiro_crew.platform.context import PlatformCompositionError

        for candidate in ({}, {"version": gov.POLICY_VERSION}):
            with pytest.raises(PlatformCompositionError):
                gov.parse_policy(candidate)
        # A minimal spelling that DOES parse is a real ceiling, not a no-op.
        assert gov.parse_policy({"version": gov.POLICY_VERSION, "boot": {}}) is not None
